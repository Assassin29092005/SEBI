...
- The draft itself can be reviewed in Hindi, not just answered in it — the
  wizard's EN/HI intake toggle extended to the generated text
...
- Each role has its own workspace: promoter (eligibility → wizard → gaps →
  draft), auditor (upload and confirm restated financials), merchant banker
  (certification, access log, service metrics). Signing in lands you on
  yours.
...
| **Vernacular draft review** (`app.generate.translate`) | **None — the one exception in this table.** Translation has no non-LLM implementation. Without a key, `GET /api/sections/{id}/translate` still answers `200`, but `translated: false` and the English original is shown untouched — never a blank page or an error, but genuinely not translated. | Translates one already-generated (and already fact-guarded) section for promoter review only — never the filed document. Every digit in the translation must appear verbatim in the approved English original; a translation that adds a number is discarded the same way generation discards ungrounded LLM text. |
