...
- `backend/app/regulation_text.py` — clause-text retrieval: resolves a checklist entry's `clause_ref` string to the real, verbatim ICDR passage it cites (`GET /api/clause-text`), sourced from `data/regulation/*.txt` — never LLM-paraphrased. Chapter IX regulation numbers resolve via an automated regex scan (globally unique, safe); Schedule VI Part A paragraph numbers resolve via a hand-verified anchor table (small numbers get reused at multiple nesting depths in that file, not safely disambiguable by regex alone) that self-checks against the live source file and fails loudly (`RegulationTextError`) rather than silently serving the wrong paragraph if the source is ever re-extracted. Anything it can't confidently resolve (a Schedule VI Part E annexure, a cross-reference to a different Schedule) is reported as `unresolved`, never guessed at.
...
- **Clause-text retrieval covers exactly what this schema cites, not the whole of Chapter IX/Schedule VI.** `app.regulation_text` indexes every Chapter IX regulation number and Schedule VI Part A paragraph the checklist actually references (pinned to `RegulationTextError`-guarded anchors for the latter) — a `clause_ref` this schema doesn't use, or a citation to a different Schedule VI Part (B/C/D/E), a different Schedule entirely, or a specific SEBI form, is reported as `unresolved`, never guessed at with a plausible-but-wrong passage. Passages over ~4000 characters are truncated with a pointer to the full text in `data/regulation/`; some Schedule VI Part A paragraphs (Financial Statements, Offering Information) genuinely run that long.
