...
- `backend/app/rate_limit.py` — in-memory sliding-window `RateLimiter`, wired in as `main.py`'s outermost middleware (auth-endpoint tier + a looser default tier, keyed by authenticated user or client IP).
...
- **Security hardening is real but scoped.** Request bodies are size-bounded (`Settings.max_request_body_bytes`, default 20 MB) via a global Content-Length check plus a bounded read loop on the upload endpoint specifically; uploaded filenames are sanitised (basename-only, control-character-stripped, length-capped) before they flow into provenance/exported documents; numeric/string inputs (eligibility figures, litigation query, fact keys/snippets, auth email/name/password) carry sanity bounds; the Gemini API key travels as a header (`x-goog-api-key`), never a URL query param, so it can't leak into request-URL logging; every request is rate-limited (see `app.rate_limit` below). Not built: virus/malware scanning of uploads, a WAF — appropriate for a single-tenant deployment behind normal network controls, not a hardened multi-tenant public endpoint.
- **Rate limiting is real but in-process, not distributed.** `app.rate_limit.RateLimiter` is a sliding-window-log limiter wired in as `app.main`'s outermost middleware — a strict tier on `/api/auth/login`/`/api/auth/register` keyed by client IP (10 requests / 5 minutes by default), a looser default tier on everything else keyed by the authenticated user when there is one, else IP (120 requests / minute by default). Both are in-memory, single-process state (`Settings.rate_limit_*` in `.env`) — correct for the single-process deployment this app targets, but horizontally scaling behind a load balancer would need a shared store (Redis or similar), since each process would otherwise enforce the limit independently and the *effective* limit would multiply by the process count.
