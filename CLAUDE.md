...
python backend/scripts/backup_data.py                 # full backup: pg_dump + uploads + audit log
                                                       # into data/backups/*.tar.gz (needs pg_dump on
                                                       # PATH or PG_DUMP_CMD set — see .env.example)
python backend/scripts/restore_data.py <archive> --yes  # DESTRUCTIVE — restores from a backup archive,
                                                       # overwriting the current DB and files
```
...
- `backend/app/backup.py` — backup & disaster recovery: `pg_dump` of Postgres + the archived-uploads and audit-log directories bundled into one timestamped `.tar.gz`, with retention pruning. `GET`/`POST /api/backup` (banker-only) list/trigger backups; restore (`backend/scripts/restore_data.py`) is CLI-only, never an HTTP endpoint — see the module docstring for why.
...
- **Postgres is single-instance, no replication.** The checked-in `docker-compose.yml` runs one dev-grade Postgres container with a local volume — replicas and streaming failover are a production infrastructure concern, not configured here. There is also no multi-tenancy: one deployment still serves one issuer's promoter/auditor/banker team (no `tenant_id`/`company_id` anywhere in the schema) — see the architecture note at the top of `backend/app/main.py`.
- **Backup is real but periodic full-dump, not continuous point-in-time recovery.** `app.backup` (`GET`/`POST /api/backup`, banker-only, plus `backend/scripts/backup_data.py`) bundles a `pg_dump` of Postgres with the archived-uploads and audit-log directories into one timestamped `.tar.gz`, with configurable retention (`Settings.backup_retention_count`, default 7). What it isn't: WAL archiving — an outage loses everything since the last successful backup run, not the last committed transaction — and nothing schedules it automatically (this app has no scheduler of its own; run `backup_data.py` from cron/Task Scheduler for a real cadence). Restore (`backend/scripts/restore_data.py`) is deliberately CLI-only with a required `--yes` flag — never an HTTP endpoint, since it overwrites live data with no undo. Requires the Postgres client tools (`pg_dump`/`psql`) as system binaries, same optional-real-capability pattern as Tesseract for OCR.
