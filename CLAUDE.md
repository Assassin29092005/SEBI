...
- `backend/app/validate/` — gap checker, contradiction detector (extract all numeric/entity claims, cross-check), boilerplate detector, adversarial examiner agent, and five deterministic, no-LLM checkers over confirmed facts only: `arithmetic.py` (Objects-of-Issue arithmetic: objects sum + GCP ≟ issue size, GCP cap per Reg. 230(2); handles a contradicted issue size without crashing), `identity_formats.py` (PAN/CIN/GSTIN/DIN regex format checks on whichever of issuer/promoter/director/subsidiary identity fields are present), `promoter_lockin.py` (minimum promoters' contribution ≥ 20% of post-issue capital per Reg. 236, and whether the disclosed lock-in distinguishes the MPC's 3-year tranche from the excess holding's mandatory 2-year/1-year split per Reg. 238 — computed at the price-band floor, the worst-case dilution scenario since the final issue price isn't fixed at draft stage), `pricing.py` (price-band structural rules per Reg. 250: cap ≤ 120% of floor, floor ≥ face value; plus face-value consistency across the share-capital history), and `rpt.py` (cross-checks company/LLP/trust-suffixed entity names mentioned in the free-text `rpt_summary` against every other disclosed related party — promoters, promoter group, group companies, subsidiaries, KMP; deliberately narrow extraction to avoid false positives on ordinary prose).
- `backend/app/review/` — banker workflow: section states, edit audit trail, certification lock.
- `backend/app/assemble/` — python-docx DRHP + abridged prospectus assembly (cover callout when confirmed issue-size facts disagree; merchant-banker disclaimer baked in) + `bundle.py` (exchange-ready ZIP: both docx + gap/contradiction/coverage/examiner/arithmetic/identity-format/promoter-lock-in/pricing/RPT JSON + full fact-provenance ledger + review audit trail + manifest, gated by the certification lock).
...
- **The four newer compliance checkers (identity formats, promoter lock-in, pricing, RPT) are deliberately narrow rather than exhaustive.** `promoter_lockin.py`'s minimum-promoters'-contribution computation uses the price-band *floor* price for new-shares-issued (the worst case for promoter dilution, since the final issue price isn't fixed at draft stage) and evaluates the promoter group in aggregate — it does not attempt to allocate the 20% MPC pool to a specific promoter's specific shares, since the regulation leaves that to the issuer's own MPC declaration. `rpt.py` only extracts entities with a recognisable company/LLP/trust suffix from the free-text `rpt_summary`; it never attempts to match person names (too high a false-positive risk against ordinary prose like "the Audit Committee"), so an undisclosed *individual* related party would not be caught. `identity_formats.py` validates whichever of PAN/CIN/GSTIN/DIN are actually present in the confirmed facts — it never flags a missing field (that's the gap checker's job) and there is no dedicated wizard prompt for GSTIN yet, though the field is honoured if supplied. None of the four call an LLM.
