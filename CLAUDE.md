...
python -m pytest tests/ -q           # full suite (256 passed, 3 skipped)
...
- **Audit log has the same growth caveat as the user store.** `app.audit.AuditLog` rewrites the whole encrypted file on every request (read-modify-write, atomic) — fine for one issuer's request volume over a drafting cycle, but it's an O(n) write per request, and the log is unbounded (no rotation/retention policy). This is exactly the class of problem a real database's append-only table exists to solve — see the Postgres-migration discussion elsewhere in this project's history. `GET /api/audit` is banker-only with no pagination beyond a `limit` cap (default 200, max 2000) — there's no UI for it yet either, only the API.
