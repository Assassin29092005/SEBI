...
- `backend/app/llm_usage.py` — LLM cost/usage tracking: every real (non-fallback) call through `app.llm.client.grounded_complete` is recorded here under a caller-supplied `feature` tag (`generate_section`, `extract_facts`, `contradiction_refine`, `semantic_check`, `examiner`), same flat-encrypted-file pattern as the audit log. `estimate_cost_usd` uses a small, dated, hardcoded pricing table — a model not in it gets `cost_usd=None`, never a silently-wrong `$0`. `GET /api/llm-usage` (banker-only) is the review surface; the banker dashboard shows a summary card + per-feature breakdown.
- `backend/app/observability.py` — error tracking: optional Sentry integration, `init_error_tracking()` called before the FastAPI app is constructed (`main.py`). Blank `SENTRY_DSN` (default) means it's never called — zero behavior change, same optional-real-integration pattern as the LLM client/litigation/OCR/backup below.
...
- **Monitoring/observability is real but scoped to what doesn't need external paid infrastructure.** Error tracking (`app.observability`) is a genuine, optional Sentry integration — set `SENTRY_DSN` to start capturing unhandled exceptions and `logger.exception()`/`error()` calls; unset (the default), it's a complete no-op, not a stub. Uptime is `GET /api/health/ready` (checks Postgres is actually reachable, unlike `/api/health`'s pure liveness check) plus `.github/workflows/uptime.yml`, a scheduled GitHub Actions job that pings it every 15 minutes and fails (which GitHub surfaces as a red run + its own notification to watchers) on a real outage — genuinely built and testable via `workflow_dispatch`, but it checks nothing until a repository variable `DEPLOYED_URL` points it at a real deployment, since this repo doesn't check one in. LLM cost/usage tracking (`app.llm_usage`) is fully real — every actual LLM call is recorded with token counts and an estimated cost — but the pricing table is a hardcoded, dated snapshot (provider pricing changes without notice and there's no live pricing API to query); an unrecognised model reports `cost_usd: null` rather than a silently wrong `$0`. None of this replaces a real APM/observability stack (structured log aggregation, dashboards, on-call paging) — that's genuine infrastructure this repo doesn't provision, same "document, don't build infra automation" stance as backup scheduling and container deployment.
