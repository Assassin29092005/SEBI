...
- `backend/app/crypto.py` — encryption at rest (Fernet/AES-128-CBC+HMAC), keyed by `Settings.encryption_key`, for archived original uploads (see below) — the only remaining filesystem consumer now that facts/review/users live in Postgres. Same ephemeral-key-with-a-warning fallback pattern as the JWT secret.
